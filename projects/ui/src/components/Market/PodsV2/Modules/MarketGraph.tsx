...
const Graph: FC<GraphProps> = ({
  ...
  const xScale = scaleLinear<number>({
    domain: [0, Math.max(maxPlaceInLine * 1.1)],
    range: [
      0, //
  ...
  const [voroNeighborIds, setVoronoiNeighborIds] = useState<Set<string>>(
    new Set()
  );
  ...
  /// Helpers
  const peerOpacity = useCallback(
    (_type: SelectedPoint['type'], _i: number) =>
      !cursorPoint?.type
        ? 1
  ...
  /// Elements
  const orderCircles = useMemo(
    () =>
      orderPositions.map((coordinate, i) => {
        const active =
          cursorPoint?.type === 'order' && i === cursorPoint?.index;
        return (
          <Circle
            key={`order-${i}`}
            cx={coordinate.x}
  ...
  const listingCircles = useMemo(
    () =>
      listingPositions.map((coordinate, i) => {
        const active =
          cursorPoint?.type === 'listing' && i === cursorPoint?.index;
        return (
          <Circle
            key={`listing-${i}`}
            cx={coordinate.x}
    ...
    <g>
      {polygons.map((polygon) => (
        <VoronoiPolygon
          ...
          fill={
            voroHoveredId &&
            (polygon.data.id === voroHoveredId ||
              voroNeighborIds.has(polygon.data.id))
              ? 'red'
              : 'transparent'
          }
          fillOpacity={
            voroHoveredId && voroNeighborIds.has(polygon.data.id) ? 0.05 : 0.2
          }
  ...
  const handleMouseMove = useCallback(
    (
      ...
      const listingIndex = findPointInCircles(
        listingPositions,
        transformedPoint
      );
        ...
        <Box sx={{ position: 'relative' }}>
          <svg width={width} height={height} ref={zoom.containerRef}>
            ...
            <rect
              ...
              css={{
                // not hovering a point, user can drag
...
const MarketGraph: FC<MarketGraphProps> = (props) => {
  ...
  return (
    <ParentSize debounceTime={100}>
      {({ width, height }) => (
        <Graph width={width} height={height} params={params} {...props}>
          {props.children}
        </Graph>
      )}
    </ParentSize>
