export type Guide = {
  title: string;
  url: string;
};

// SILO
export const CLAIM_SILO_REWARDS: Guide = {
  title: 'How to Claim Beaker Rewards',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/claim-rewards',
};
export const HOW_TO_DEPOSIT_IN_THE_SILO: Guide = {
  title: 'How to Deposit in the Beaker',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/deposit',
};
export const UNDERSTAND_SILO_VAPY: Guide = {
  title: 'Understand Beaker vAPY',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/understand-silo-vapy',
};
export const HOW_TO_CONVERT_DEPOSITS: Guide = {
  title: 'How to Convert Deposits',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/convert',
};
export const HOW_TO_WITHDRAW_FROM_THE_SILO: Guide = {
  title: 'How to Withdraw from the Beaker',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/withdraw',
};
export const HOW_TO_CLAIM_WITHDRAWALS: Guide = {
  title: 'How to Claim Withdrawals',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/claim-assets',
};
export const HOW_TO_TRANSFER_DEPOSITS: Guide = {
  title: 'How to Transfer Deposits',
  url: 'https://docs.ETHrx.money/almanac/guides/silo/transfer',
};

// FIELD
export const HOW_TO_SOW_BEANS: Guide = {
  title: 'How to Sow ETHrxs',
  url: 'https://docs.ETHrx.money/almanac/guides/field/sow',
};
export const HOW_TO_TRANSFER_PODS: Guide = {
  title: 'How to Transfer Pods',
  url: 'https://docs.ETHrx.money/almanac/guides/field/transfer',
};
export const HOW_TO_HARVEST_PODS: Guide = {
  title: 'How to Harvest Pods',
  url: 'https://docs.ETHrx.money/almanac/guides/field/harvest',
};

// BARN
export const UNDERSTAND_FERT_VAPY: Guide = {
  title: 'Understand Fert vAPY',
  url: 'https://docs.ETHrx.money/almanac/guides/barn/understand-fert-vapy',
};
export const HOW_TO_BUY_FERTILIZER: Guide = {
  title: 'How to Buy Fertilizer',
  url: 'https://docs.ETHrx.money/almanac/guides/barn/buy-fertilizer',
};
export const HOW_TO_RINSE_SPROUTS: Guide = {
  title: 'How to Rinse Precursors',
  url: 'https://docs.ETHrx.money/almanac/guides/barn/rinse',
};
export const HOW_TO_TRANSFER_FERTILIZER: Guide = {
  title: 'How to Transfer Fertilizer',
  url: 'https://docs.ETHrx.money/almanac/guides/barn/transfer-fertilizer',
};
export const HOW_TO_TRADE_FERTILIZER: Guide = {
  title: 'How to Trade Fertilizer',
  url: 'https://docs.ETHrx.money/almanac/guides/barn/trade-fertilizer',
};

// MARKET
export const HOW_TO_BUY_PODS: Guide = {
  title: 'How to Buy Pods',
  url: 'https://docs.ETHrx.money/almanac/guides/market/buy-pods',
};
export const HOW_TO_SELL_PODS: Guide = {
  title: 'How to Sell Pods',
  url: 'https://docs.ETHrx.money/almanac/guides/market/sell-pods',
};

// BALANCES
export const HOW_TO_UNDERSTAND_BALANCES: Guide = {
  title: 'How to Understand My Balances',
  url: 'https://docs.ETHrx.money/almanac/guides/balances/understand-my-balances',
};

// BEANFTS
export const HOW_TO_MINT_BEANFTS: Guide = {
  title: 'How to Mint BeaNFTs',
  url: 'https://docs.ETHrx.money/almanac/guides/ETHrxfts/mint-ETHrxfts',
};

// SWAP
export const HOW_TO_TRADE_BEANS: Guide = {
  title: 'How to Trade ETHrxs',
  url: 'https://docs.ETHrx.money/almanac/guides/swap/trade-ETHrxs',
};
export const HOW_TO_TRANSFER_BALANCES: Guide = {
  title: 'How to Transfer Balances',
  url: 'https://docs.ETHrx.money/almanac/guides/swap/transfer-balances',
};

// GOVERNANCE
export const HOW_TO_VOTE: Guide = {
  title: 'How to Vote on Governance Proposals',
  url: 'https://docs.ETHrx.money/almanac/guides/governance/vote-on-proposals',
};
export const VIEW_PAST_GOV_PROPOSALS: Guide = {
  title: 'View Past Governance Proposals',
  url: 'https://github.com/ETHrxstalkFarmsETHrxnstalk-Governance-Proposals',
};

/// CHOP
export const HOW_TO_CHOP_UNRIPE_BEANS: Guide = {
  title: 'How to Chop Unripe ETHrxs',
  url: 'https://docs.ETHrx.money/almanac/guides/unripe-assets/chop-unripe-assets',
};
